...
//		natcasesort($alldirs);
	...
	private function getMarkerFromFile($file, $userId) {
			...
			foreach ($track->trkseg as $segment) {
				...
				foreach ($segment->trkpt as $point) {
					...
					if ($lastShortPoint !== null) {
						...
						// we add it
						if ($this->processService->distance($lastShortPoint, $point) > $DISTANCE_BETWEEN_SHORT_POINTS) {
							$shortPointList[] = [$pointlat, $pointlon];
							$lastShortPoint = $point;
						}
					}
		...
		$result = sprintf('[%s, %s, "%s", "%s", %.3f, %s, "%s", "%s", %s, %.2f, %s, %s, %s, %.2f, %s, %s, %s, %.6f, %.6f, %.6f, %.6f, %s, %s, "%s", "%s", %.2f]',
			$lat,
			$lon,
			$this->toolsService->encodeURIComponent($gpx_relative_dir),
			$this->toolsService->encodeURIComponent($name),
			$total_distance,
			$total_duration,
			$date_begin,
			$date_end,
	...
	private function getDistanceFilteredPoints($points) {
		...
		if (count($points) > 0) {
			...
			$lastPoint = $points[0];
			foreach ($points as $point) {
				if ($this->processService->distance($lastPoint, $point) >= $DISTANCE_THRESHOLD) {
	...
	private function getMaxSpeed($points) {
		...
		if (count($points) > 0) {
			...
			foreach ($points as $point) {
				...
				if ($timeDelta > 0) {
					$distance = $this->processService->distance($point, $lastPoint);
